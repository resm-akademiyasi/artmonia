...
const TeachersManager = () => {
  ...
  const handleReorder = async (reordered: Teacher[]) => {
    setTeachers(reordered);
    const updates = reordered.map((item, index) =>
      supabase.from("teachers").update({ display_order: index }).eq("id", item.id)
    );
        ...
        <div className="rounded-xl border border-border bg-card p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} placeholder="Müəllimin adı" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Titul / Vəzifə</Label>
              <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Baş müəllim, Rəsm ustası..." />
            </div>
          </div>

          <div>
            <Label className="text-xs text-muted-foreground">Haqqında</Label>
            <Textarea value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} placeholder="Müəllim haqqında qısa məlumat..." rows={3} />
          </div>

          <div>
            <Label className="text-xs text-muted-foreground">Şəkil</Label>
            <div className="flex gap-2 mt-1">
              <Input value={form.photo_url} onChange={(e) => setForm({ ...form, photo_url: e.target.value })} placeholder="URL və ya yüklə" className="flex-1" />
            ...
            {form.photo_url && <img src={form.photo_url} alt="" className="mt-2 h-24 w-24 rounded-xl object-cover" />}
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2 mt-1">
              <Input
                ...
                className="flex-1"
              />
              <Button type="button" variant="outline" size="sm" onClick={addSpecialty}>Əlavə et</Button>
            </div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button size="sm" onClick={handleSubmit} disabled={uploading}>{uploading ? "Yüklənir..." : "Əlavə et"}</Button>
          </div>
        </div>
      )}
        ...
        <SortableList
          ...
          renderItem={(t) => (
            <div className="flex items-center gap-4 rounded-xl border border-border bg-card p-4">
              ...
              <button onClick={() => togglePublish(t.id, t.is_published)} className="text-muted-foreground hover:text-accent transition-colors">
                {t.is_published ? <Eye size={16} /> : <EyeOff size={16} />}
              </button>
              <button onClick={() => deleteTeacher(t.id)} className="text-muted-foreground hover:text-destructive transition-colors">
                <Trash2 size={16} />
              </button>
            </div>
          )}
